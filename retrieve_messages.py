...
def retrieve(phone_number):
    if not phone_number:
        raise ValueError("Set a phone number in Settings before retrieving messages")
    ...
    with sqlite3.connect(db) as conn, open(out, "w", newline="", encoding="utf-8") as f:
        ...
        q = """SELECT datetime(m.date/1e9 + 978307200, 'unixepoch', 'localtime'), CASE m.is_from_me WHEN 1 THEN 'Me' ELSE h.id END, m.text, m.attributedBody
               ...
               WHERE h.id LIKE ? OR h2.id LIKE ? GROUP BY m.ROWID ORDER BY m.date ASC;"""
        phone_pattern = f"%{phone_number}%"
        for dt, snd, t, b in conn.execute(q, (phone_pattern, phone_pattern)):
...
if __name__ == "__main__":
    raise SystemExit("Run message retrieval from the app's Settings page")
